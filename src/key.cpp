...
bool CKey::Derive(CKey& keyChild, ChainCode &ccChild, unsigned int nChild, const ChainCode& cc) const {
    ...
    if ((nChild >> 31) == 0) { // < 2^32
        CPubKey pubkey = GetPubKey();  //WA2301:先根据私钥计算出公钥        
    ...
    memcpy(ccChild.begin(), vout.data()+32, 32); //后32byte作为链码

    memcpy((unsigned char*)keyChild.begin(), begin(), 32);

    bool ret = secp256k1_ec_privkey_tweak_add(secp256k1_context_sign, (unsigned char*)keyChild.begin(), vout.data()); //WA2301:算术加法，如果溢出则减取阶数n
...
void CExtKey::SetMaster(const unsigned char *seed, unsigned int nSeedLen) {
    static const unsigned char hashkey[] = {'B','i','t','c','o','i','n',' ','s','e','e','d'}; //Bitcoin seed
    //std::vector<unsigned char, secure_allocator<unsigned char>> vout(64);
	//! Debug only	
	std::vector<unsigned char> vout(64);
